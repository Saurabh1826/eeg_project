...
def split_data(data, fs, clip_len, overlap, start_time=0) :
  ...
  times += clip_len
  clip_len, overlap, stride = int(clip_len * fs), int(overlap * fs), int(stride * fs)
...
def create_notch_filter(order, low_cutoff, high_cutoff, fs) :
  ...
  """
  b, a = sig.butter(N=order, Wn=[low_cutoff, high_cutoff], btype='bandstop', fs=fs)
  return b, a
...
def resample(x, sfreq_original, sfreq_new) :
  ...
  time_len = x.shape[-1] / sfreq_original # Length of time of the input signal is num_samples / sfreq
  num_samples_new = int(time_len * sfreq_new) # Number of samples in resampled signal
  return sig.resample(x, num_samples, axis=-1) # Resample signal along last dimension of x.shape
...
def plv(data) :
  ...
  for i in range(n) :
    for j in range(n) :
      plvs[i][j] = (1 / N) * np.abs(np.matmul(phases[i], np.conj(phases[j].T)))
...
def coherence(data, fs) :
  ...
  for i in range(N) :
    for j in range(i+1, N) :
      ...
      res = f[1] - f[0]
      matrix[i][j] = simps(c, dx=res) / (len(f) * res) # Normalize area so it is between 0 and 1 (so diagonal entries will be 1's)
...
def data_to_graphs(clips, graph_func, fs) :
  ...
  if (graph_func == 'coherence') :
    graphs = Parallel(n_jobs=2)(delayed(coherence)(clips[:, i], fs) for i in range(len(clips[0])))
  elif (graph_func == 'plv') :
    graphs = Parallel(n_jobs=2)(delayed(plv)(clips[:, i]) for i in range(len(clips[0])))
...
def create_bipolar_montage(data, ch_names) :
  ...
  bipolar_montage['fp1-f7'] = data[name_to_index['fp1']] - data[name_to_index['f7']]
  bipolar_montage['f7-t3'] = data[name_to_index['f7']] - data[name_to_index['t3']]
  bipolar_montage['t3-t5'] = data[name_to_index['t3']] - data[name_to_index['t5']]
  bipolar_montage['t5-o1'] = data[name_to_index['t5']] - data[name_to_index['o1']]

  bipolar_montage['fp1-f3'] = data[name_to_index['fp1']] - data[name_to_index['f3']]
  bipolar_montage['f3-c3'] = data[name_to_index['f3']] - data[name_to_index['c3']]
  bipolar_montage['c3-p3'] = data[name_to_index['c3']] - data[name_to_index['p3']]
  bipolar_montage['p3-o1'] = data[name_to_index['p3']] - data[name_to_index['o1']]

  bipolar_montage['fp2-f4'] = data[name_to_index['fp2']] - data[name_to_index['f4']]
  bipolar_montage['f4-c4'] = data[name_to_index['f4']] - data[name_to_index['c4']]
  bipolar_montage['c4-p4'] = data[name_to_index['c4']] - data[name_to_index['p4']]
  bipolar_montage['p4-o2'] = data[name_to_index['p4']] - data[name_to_index['o2']]

  bipolar_montage['fp2-f8'] = data[name_to_index['fp2']] - data[name_to_index['f8']]
  bipolar_montage['f8-t4'] = data[name_to_index['f8']] - data[name_to_index['t4']]
  bipolar_montage['t4-t6'] = data[name_to_index['t4']] - data[name_to_index['t6']]
  bipolar_montage['t6-o2'] = data[name_to_index['t6']] - data[name_to_index['o2']]

  bipolar_montage['fz-cz'] = data[name_to_index['fz']] - data[name_to_index['cz']]
  bipolar_montage['cz-pz'] = data[name_to_index['cz']] - data[name_to_index['pz']]
...
def plot_eeg(data, sfreq, start=0, end=15, indices=None, ch_names=None, padding=0, figsize = (24,8)) :
  ...
  arr = np.copy(data) # arr is the variable for the copy of the data that will be used in the function
  ...
  times = np.arange(0, len(arr[0]), 1) # Creates an array [0,1,2,...,len(data[0])]
  times = times / sfreq # Divide all entries in times by sample frequency to obtain the desired times array
